// ════════════════════════════════════════════════════════════════════
// voice.js — DRISHTI Voice System v8.0 (FULL VERSION + STABLE FIX)
// ════════════════════════════════════════════════════════════════════
...
// 🛑 Fix: Cleanup old wake instances to stop clicking
if (_wakeRec) {
...
try {
  ...
  _wakeRec.onerror = (e) => {
    ...
    if (e.error !== "not-allowed" && e.error !== "aborted") _scheduleWakeRestart();
  };

  _wakeRec.onend = () => {
    _wakeActive = false;
    _updateWakeIndicator(false);
    if (_wakeEnabled && !_listening && !_sending) _scheduleWakeRestart();
  };
...
_wakeTimer = setTimeout(() => {
  _wakeTimer = null;
  if (_wakeEnabled && !_listening && !_sending && !_wakeActive) startWakeWord();
}, CFG.WAKE_RESTART_MS);
}
...
mic.innerHTML = "👁️";
setTimeout(() => { mic.style.background = "#DC2626"; mic.innerHTML = "🔴"; }, 500);
}
...
if (!dot) {
  ...
  if ($("mic")) $("mic").parentNode.appendChild(dot);
}
...
// ── Main Listening Engine (Fixed Sensitivity) ──
...
_rec.interimResults = true; // 🎤 Fix: Real-time update enabled
